...
const TIME_BLOCK_CLASSES: Record<string, string> = {
...
function normalizeTask(rawTask: unknown, defaultTimeBlockId?: string): TodayTask | null {
  if (!isRecord(rawTask)) {
    return null;
  }
  ...
  return {
    ...
    milestoneTitle:
      readString(rawTask, 'milestoneTitle', 'milestone_title') ??
...
function normalizeTimeBlock(rawBlock: unknown): TodayTimeBlock | null {
  ...
  return {
    ...
    tasks: nestedTasks as TodayTask[]
  };
}
...
function attachTasksToBlocks(blocks: TodayTimeBlock[], tasks: TodayTask[]): TodayTimeBlock[] {
  const blockTasks = new Map<string, TodayTask[]>();
...
function normalizeTodayViewResponse(payload: unknown, todayKey: string): TodayViewPayload {
  ...
  const normalizedBlocks = rawTimeBlocks.map(normalizeTimeBlock).filter(Boolean) as TodayTimeBlock[];
  const normalizedTasks = rawTasks.map((task) => normalizeTask(task)).filter(Boolean) as TodayTask[];
...
function buildFallbackTodayPayload(todayKey: string): TodayViewPayload {
  const yesterday = shiftDate(todayKey, -1);
  const completedAt = new Date().toISOString();

  const timeBlocks: TodayTimeBlock[] = [
    {
...
async function fetchTodayView(todayKey: string): Promise<TodayViewPayload> {
  try {
...
function applyTaskOverrides(tasks: TodayTask[], overrides: Record<string, TaskOverride>): TodayTask[] {
...
function getTaskCompletionDate(task: TodayTask): string | null {
...
function getTimelineBounds(timeBlocks: TodayTimeBlock[]): { start: number; end: number } {
  if (!timeBlocks.length) {
    return { start: 8 * 60, end: 18 * 60 };
  }

  const starts = timeBlocks.map((block) => parseTimeToMinutes(block.startTime)).filter((value): value is number => value !== null);
  const ends = timeBlocks.map((block) => parseTimeToMinutes(block.endTime)).filter((value): value is number => value !== null);
...
function getExecutorLabel(executorType: string): { label: string; icon: JSX.Element } {
  const normalizedType = executorType.toLowerCase();

  if (normalizedType === 'agent') {
    return { label: 'Agent', icon: <Bot className="h-3.5 w-3.5" /> };
  }

  if (normalizedType === 'human') {
    return { label: 'You', icon: <UserRound className="h-3.5 w-3.5" /> };
  }

  return { label: 'Mixed', icon: <CircleHelp className="h-3.5 w-3.5" /> };
}
...
function TaskRow({ task, onToggle, onOpenTask, overdue = false }: TaskRowProps) {
  ...
  return (
    <div
      className={cn(
        ...
        overdue ? 'border-error/60 bg-error/10' : 'border-border bg-bg-raised',
        done ? 'opacity-75' : ''
      )}
    >
      <div className="flex items-start gap-3">
        <button
          ...
          className={cn(
            ...
            done ? 'border-success bg-success text-success-subtle' : 'border-border-strong bg-bg-surface hover:border-primary-400'
          )}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={cn(
                'text-left text-sm font-semibold text-text-primary hover:text-primary-300',
                done ? 'line-through text-text-secondary hover:text-text-secondary' : ''
              )}
            >
              {task.title}
            </button>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-text-secondary">
            <span className="inline-flex items-center gap-1 rounded-full bg-bg-overlay px-2 py-0.5">
              {executor.icon}
              <span>{executor.label}</span>
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-bg-overlay px-2 py-0.5">
              <Clock3 className="h-3.5 w-3.5" />
              <span>{formatDuration(task.durationEst)}</span>
            </span>
...
function TimelineTaskGroup({ block, top, height, onToggleTask, onOpenTask }: TimelineTaskGroupProps) {
  const blockClass = TIME_BLOCK_CLASSES[block.blockType] ?? 'border-l-area-8 bg-area-8/10';

  return (
    <article
      className={cn('absolute left-20 right-0 rounded-lg border border-border border-l-4 p-3', blockClass)}
      style={{ top, minHeight: Math.max(88, height) }}
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        <p className="text-sm font-semibold text-text-primary">{block.title}</p>
        <span className="text-xs font-medium text-text-secondary">{formatBlockTime(block.startTime, block.endTime)}</span>
      </div>

      {block.tasks.length ? (
        <div className="space-y-2">
          {block.tasks.map((task) => (
            <TaskRow key={task.id} task={task} onToggle={onToggleTask} onOpenTask={onOpenTask} />
          ))}
        </div>
      ) : (
        <p className="text-xs text-text-secondary">No tasks in this block.</p>
      )}
    </article>
  );
}
...
export default function TodayView() {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ taskId, status }: { taskId: string; status: string }) => patchTaskStatus(taskId, status),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: [TODAY_QUERY_KEY, todayKey] });
    }
  });
  ...
  const scheduledTodayTasks = useMemo(() => {
    return sortTasks(
      pendingTasks.filter((task) => normalizeDateKey(task.scheduledDate) === todayKey)
    );
  }, [pendingTasks, todayKey]);

  const scheduledBySlot = useMemo(() => {
    const groups = new Map<string, { order: number; tasks: TodayTask[] }>();
  ...
  const unscheduledTasks = useMemo(() => {
    return sortTasks(
      pendingTasks.filter((task) => normalizeDateKey(task.dueDate) === todayKey && !task.timeBlockId)
    );
  }, [pendingTasks, todayKey]);

  const completedTodayTasks = useMemo(() => {
    return sortTasks(
      tasks.filter((task) => getTaskCompletionDate(task) === todayKey)
    );
  }, [tasks, todayKey]);
  ...
  return (
    <div className="mx-auto w-full max-w-[800px] space-y-4 pb-6">
      <Card>
        <CardHeader>
          <CardTitle>Today</CardTitle>
          <CardDescription>{todayDateLabel}</CardDescription>
        </CardHeader>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle className="text-sm tracking-[0.16em] text-error">OVERDUE</CardTitle>
          <CardDescription>Due before today and not marked done.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? <p className="text-sm text-text-secondary">Loading tasks...</p> : null}
          {!isLoading && !overdueTasks.length ? <p className="text-sm text-text-secondary">No overdue tasks.</p> : null}
          {overdueTasks.length ? (
            <div className="space-y-2">
              {overdueTasks.map((task) => (
                <TaskRow key={task.id} task={task} overdue onToggle={handleToggleTask} onOpenTask={openTask} />
              ))}
            </div>
          ) : null}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm tracking-[0.16em]">TIME BLOCK TIMELINE</CardTitle>
          <CardDescription>Vertical timeline grouped by time block type.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="relative rounded-lg border border-border bg-bg-surface p-3">
            <div className="relative" style={{ height: `${timelineHeight}px` }}>
              {hourMarkers.map((marker) => (
                <div key={marker.label} className="absolute left-0 right-0" style={{ top: marker.top }}>
                  <div className="flex items-center gap-2">
                    <span className="w-14 text-[11px] font-mono text-text-tertiary">{marker.label}</span>
                    <div className="h-px flex-1 bg-border-subtle" />
                  </div>
                </div>
              ))}

              <div className="absolute left-20 right-0 z-20" style={{ top: nowLineTop }} data-testid="today-now-line">
                <div className="relative h-px bg-error">
                  <span className="absolute -top-3 right-0 rounded bg-error px-2 py-0.5 text-[10px] font-semibold text-white">
                    {formatMinutesAsTime(now.getHours() * 60 + now.getMinutes())}
                  </span>
                </div>
              </div>
              ...
              {timeBlocks.length ? (
                ...
                <p className="absolute left-20 top-3 text-sm text-text-secondary">No time blocks for today.</p>
              )}
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm tracking-[0.16em]">SCHEDULED</CardTitle>
          <CardDescription>Tasks scheduled for today, grouped by slot.</CardDescription>
        </CardHeader>
        <CardContent>
          {!scheduledBySlot.length ? <p className="text-sm text-text-secondary">No scheduled tasks for today.</p> : null}
          {scheduledBySlot.length ? (
            <div className="space-y-3">
              {scheduledBySlot.map((group) => (
                <div key={group.slotLabel} className="space-y-2 rounded-lg border border-border p-3">
                  <p className="text-xs font-semibold tracking-[0.08em] text-text-secondary">{group.slotLabel}</p>
                  <div className="space-y-2">
                    {group.tasks.map((task) => (
                      <TaskRow key={task.id} task={task} onToggle={handleToggleTask} onOpenTask={openTask} />
                    ))}
                  </div>
                </div>
              ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm tracking-[0.16em]">UNSCHEDULED</CardTitle>
          <CardDescription>Due today with no assigned time block.</CardDescription>
        </CardHeader>
        <CardContent>
          {!unscheduledTasks.length ? <p className="text-sm text-text-secondary">No unscheduled tasks due today.</p> : null}
      ...
      <Card>
        <CardHeader className="mb-0 flex flex-row items-center justify-between space-y-0">
          <div className="space-y-1">
            <CardTitle className="text-sm tracking-[0.16em]">COMPLETED TODAY</CardTitle>
            <CardDescription>Completed tasks stay collapsed by default.</CardDescription>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span>{showCompleted ? 'Collapse' : 'Expand'}</span>
            <span>({completedTodayTasks.length})</span>
          </button>
        </CardHeader>
        {showCompleted ? (
          <CardContent>
            {!completedTodayTasks.length ? <p className="text-sm text-text-secondary">No completed tasks yet.</p> : null}
