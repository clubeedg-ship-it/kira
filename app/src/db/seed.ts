...
async function countScopedRows(tableName: string, userId: string): Promise<number> {
...
async function countAllRows(tableName: string): Promise<number> {
  const result = await pool.query<{ count: string }>(
...
export async function runSeed() {
  ...
  await db.transaction(async (tx) => {
    ...
    const objectiveByTitle = Object.fromEntries(
      seededObjectives.map((objective) => [objective.title, objective.id]),
    );
    ...
    const milestoneByTitle = Object.fromEntries(
      seededMilestones.map((milestone) => [milestone.title, milestone.id]),
    );
    ...
    const timeBlockByTitle = Object.fromEntries(
      seededTimeBlocks.map((timeBlock) => [timeBlock.title, timeBlock.id]),
    );
