...
import { tasks, extractedSuggestions, projects, areas } from '../db/schema';
 ...
 */
export async function extractTasksLLM(
  userId: string,
  ...
  if (trivialPatterns.test(assistantContent.trim())) return null;
  if (userContent && trivialPatterns.test(userContent.trim())) return null;
  ...
  try {
    ...
    console.error('[task-extract] Extraction error:', (err as Error).message);
    return null;
  }
}
 ...
 */
async function storeExtractedTasks(
  userId: string,
  conversationId: string,
  items: ExtractedTask[],
): Promise<void> {
  ...
  for (const item of items) {
    try {
        ...
        if (item.area) {
          const [project] = await db
            ...
            .where(and(
              ...
              sql`LOWER(${projects.title}) LIKE LOWER(${'%' + item.area + '%'})`,
            ))
            .limit(1);
      ...
      console.error(`[task-extract] Failed to store item "${item.title}":`, (err as Error).message);
    }
  }
}
