...
function resolveFullCheckPhase(now: Date, hint: FullCheckPhase | 'auto'): FullCheckPhase {
  if (hint !== 'auto') {
    return hint;
  }
...
async function upsertDailyReview(input: DailyReviewInput): Promise<string> {
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        gte(reviews.scheduled, dayStart.toISOString()),
        lt(reviews.scheduled, nextDay.toISOString()),
      ),
    )
...
async function checkStaleInputQueue(userId: string, now: Date): Promise<number> {
  ...
  const [countRow] = await db
    ...
    .where(
      and(
        eq(inputQueue.userId, userId),
        eq(inputQueue.status, 'pending'),
        lt(inputQueue.createdAt, staleThresholdIso),
      ),
    );
  ...
  if (staleCount > 0) {
    emitNotification(
      userId,
      'Stale inbox items',
      `${staleCount} input queue item(s) have been pending for more than 48 hours.`,
      1,
      '/inbox',
    );
  }
...
async function unblockResolvedDependencies(userId: string): Promise<number> {
  ...
  for (const waitingTask of waitingRows) {
    ...
    const blockerRows = await db
      ...
      .innerJoin(
        tasks,
        and(eq(tasks.id, dependencies.blockerId), eq(tasks.userId, userId)),
      )
      .where(
        and(
          ...
          eq(dependencies.blockedId, waitingTask.id),
        ),
      );
    ...
    const [updated] = await db
      ...
      .where(
        and(eq(tasks.userId, userId), eq(tasks.id, waitingTask.id), eq(tasks.status, 'waiting')),
      )
...
async function checkDeadlineWarnings(
  ...
  today: string,
  tomorrow: string,
): Promise<{ dueTodayWarnings: number; dueTomorrowWarnings: number }> {
  ...
  if (dueTodayWarnings > 0) {
    emitNotification(
      userId,
      'Tasks due today',
      `${dueTodayWarnings} task(s) are due today.`,
      1,
      '/today',
    );
  }
  ...
  if (dueTomorrowWarnings > 0) {
    emitNotification(
      userId,
      'Tasks due tomorrow',
      `${dueTomorrowWarnings} task(s) are due tomorrow.`,
      2,
      '/today',
    );
  }
...
async function checkAgentStuck(userId: string, now: Date): Promise<number> {
  ...
  const stuckAgents = await db
    ...
    .where(
      and(
        ...
        lt(agents.updatedAt, staleThresholdIso),
      ),
    )
  ...
  for (const stuckAgent of stuckAgents) {
    console.warn(
      `[heartbeat] user=${userId} agent=${stuckAgent.id} "${stuckAgent.name}" may be stuck (working since ${stuckAgent.updatedAt}).`,
    );
  }
...
async function checkOverdueTasks(userId: string, today: string): Promise<number> {
  ...
  if (overdueTasks > 0) {
    emitNotification(
      userId,
      'Overdue tasks',
      `${overdueTasks} task(s) are overdue and still not complete.`,
      0,
      '/today',
    );
  }
...
async function runQuickChecks(userId: string, now: Date): Promise<QuickCheckMetrics> {
  const today = toDateOnly(now);
  const tomorrow = toDateOnly(addDays(startOfDayUtc(now), 1));

  const [staleInputItems, unblockedTasks, warnings, stuckAgents, overdueTasks] =
    await Promise.all([
...
async function runMorningBrief(userId: string, now: Date): Promise<FullCheckMetrics> {
  ...
  const [
    completedYesterdayCountRow,
    todayTaskCountRow,
    pendingInputCountRow,
    overdueCountRow,
    keyResultRows,
    ...
    db
      ...
      .where(
        and(
          eq(tasks.userId, userId),
          gte(tasks.completedAt, yesterdayStart.toISOString()),
          lt(tasks.completedAt, todayStart.toISOString()),
        ),
      )
      .then((rows) => rows[0]),
    db
      ...
      .where(
        and(
          eq(tasks.userId, userId),
          or(eq(tasks.scheduledDate, todayDate), eq(tasks.dueDate, todayDate)),
          ne(tasks.status, 'done'),
          ne(tasks.status, 'cancelled'),
        ),
      )
  ...
  const krProgress =
    keyResultRows.length === 0
      ? 0
      : Math.round(
          keyResultRows.reduce((total, row) => {
...
async function runEveningClose(userId: string, now: Date): Promise<FullCheckMetrics> {
  const todayStart = startOfDayUtc(now);
  const tomorrowStart = addDays(todayStart, 1);
  const todayDate = toDateOnly(now);
  ...
  const unfinishedRows = await db
    ...
    .where(
      and(
        ...
        ne(tasks.status, 'done'),
        ne(tasks.status, 'cancelled'),
        or(lte(tasks.scheduledDate, todayDate), lte(tasks.dueDate, todayDate)),
      ),
    )
    .orderBy(desc(tasks.priorityScore), asc(tasks.priority), asc(tasks.createdAt))
    .limit(300);
  ...
  if (unfinishedRows.length > 0) {
    ...
    const updated = await db
      ...
      .where(
        and(
          eq(tasks.userId, userId),
          inArray(tasks.id, unfinishedIds),
          ne(tasks.status, 'done'),
          ne(tasks.status, 'cancelled'),
        ),
      )
  ...
  const [completedTodayCountRow, pendingInputCountRow, overdueCountRow] = await Promise.all([
    db
      ...
      .where(
        and(
          eq(tasks.userId, userId),
          gte(tasks.completedAt, todayStart.toISOString()),
          lt(tasks.completedAt, tomorrowStart.toISOString()),
        ),
      )
      .then((rows) => rows[0]),
    db
...
async function runFullChecks(
  userId: string,
  now: Date,
  phaseHint: FullCheckPhase | 'auto',
): Promise<FullCheckMetrics> {
...
function assertCheckType(value: string): HeartbeatCheckType {
...
export async function processHeartbeatJob(job: Job<HeartbeatJobData>): Promise<HeartbeatRunResult> {
