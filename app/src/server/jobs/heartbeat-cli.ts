...
type FullCheckMode = 'auto' | 'morning' | 'evening';

function parseUserIdArg(argv: string[]): string | null {
...
function parseModeArg(argv: string[]): FullCheckMode {
...
async function resolveUserIds(argv: string[]): Promise<string[]> {
...
async function main(): Promise<void> {
  ...
  for (const userId of userIds) {
    ...
    console.log(
      `[heartbeat:once] user=${userId} full-check complete | overdue=${result.quick.overdueTasks} review=${result.full?.reviewId ?? 'none'}`,
    );
  }
