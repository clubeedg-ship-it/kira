...
const MODEL_PRICING_PER_MILLION: Array<{
  matcher: RegExp;
  input: number;
  output: number;
}> = [
...
function normalizeQueueType(raw: string): string {
...
async function generateAgentOutput(params: {
  ...
  userPrompt: string;
  taskTitle: string;
}): Promise<{ text: string; tokensUsed: number; costUsd: number; model: string; isFallback: boolean }> {
  ...
  (finalError as any).tokensUsed = cumulativeTokensUsed;
  (finalError as any).costUsd = cumulativeCostUsd;
  throw finalError;
}
...
async function emitPendingInputCount(userId: string): Promise<void> {
...
async function loadContext(userId: string, agentId: string, taskId: string): Promise<LoadedContext> {
...
export async function processAgentWorkJob(
  job: Job<AgentWorkJobData>,
): Promise<{ taskId: string; agentId: string; finalStatus: string }> {
  ...
  let partialModel: string | null = null;
  ...
  try {
    ...
    currentAgentStatus = await setAgentStatus(
      userId,
      agentId,
      'working',
      currentAgentStatus,
