...
dependenciesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    const conditions: SQL<unknown>[] = [eq(dependencies.userId, req.userId)];
    ...
    if (taskId) {
      ...
      conditions.push(or(eq(dependencies.blockedId, taskId), eq(dependencies.blockerId, taskId)) as SQL<unknown>);
    }
...
dependenciesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(dependencies).values(insertValues as any).returning();
...
dependenciesRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [deleted] = await db
      .delete(dependencies)
      .where(and(eq(dependencies.id, req.params.id as string), eq(dependencies.userId, req.userId)))
      .returning();
