...
contextMetricsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const conditions = [];
    if (userId) conditions.push(eq(contextMetrics.userId, userId));
    if (from) conditions.push(gte(contextMetrics.createdAt, from));
    if (to) conditions.push(lte(contextMetrics.createdAt, to));
    ...
    const trend = await db
      .select({
        date: sql<string>`date_trunc('day', ${contextMetrics.createdAt})::date`.as('date'),
        avgEfficiency: avg(contextMetrics.efficiencyScore),
        avgInputTokens: sql<number>`avg(${contextMetrics.systemPromptTokens} + ${contextMetrics.historyTokens})`,
        requests: count(),
      })
      .from(contextMetrics)
      .where(where)
