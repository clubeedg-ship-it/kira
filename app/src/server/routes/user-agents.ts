...
userAgentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!name) { validationError(res, 'name is required'); return; }
    if (!systemPrompt) { validationError(res, 'systemPrompt is required'); return; }
...
userAgentsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!isUuid(id)) { validationError(res, 'id must be a valid UUID'); return; }
    ...
    if (!existing) { notFound(res, 'Agent not found'); return; }

    const body = asRecord(req.body);
    const updates: Record<string, unknown> = { updatedAt: sql`now()` };

    if (body.name !== undefined) updates.name = getTrimmedString(body.name) || existing.name;
    if (body.type !== undefined) updates.type = getTrimmedString(body.type) || existing.type;
    if (body.model !== undefined) updates.model = getTrimmedString(body.model) || existing.model;
    ...
    if (body.tools !== undefined) updates.tools = Array.isArray(body.tools) ? body.tools : existing.tools;
    if (body.schedule !== undefined) updates.schedule = getTrimmedString(body.schedule) || null;
...
userAgentsRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (deleted.length === 0) { notFound(res, 'Agent not found'); return; }
...
userAgentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!agent) { notFound(res, 'Agent not found'); return; }
