...
const cache: {
  runtime: CacheEntry<RuntimeSkill[]> | null;
  marketplace: CacheEntry<MarketplaceSkill[]> | null;
} = { runtime: null, marketplace: null };

const RUNTIME_TTL = 30_000;      // 30s
const MARKETPLACE_TTL = 300_000; // 5min

// ─── Types ──────────────────────────────────────────────────────────────────
interface RuntimeSkillRaw {
  name: string;
  missing?: {
    bins: string[];
    anyBins: string[];
...
interface RuntimeSkill {
  ...
  name: string;
  source: 'runtime';
  status: 'ready' | 'missing-deps' | 'blocked';
  eligible: boolean;
  missing?: RuntimeSkillRaw['missing'];
  installHints?: RuntimeSkillRaw['install'];
}
...
// ─── Helpers ────────────────────────────────────────────────────────────────
async function getOpenClawSkills(): Promise<RuntimeSkill[]> {
  if (cache.runtime && Date.now() - cache.runtime.ts < RUNTIME_TTL) return cache.runtime.data;
  ...
  try {
    ...
    const eligible = new Set<string>(parsed.eligible || []);
    const disabled = new Set<string>(parsed.disabled || []);
    const blocked = new Set<string>(parsed.blocked || []);
    ...
    if (cache.runtime) return cache.runtime.data; // stale is better than nothing
    throw err;
  }
}

async function getMarketplaceSkills(): Promise<MarketplaceSkill[]> {
  if (cache.marketplace && Date.now() - cache.marketplace.ts < MARKETPLACE_TTL) return cache.marketplace.data;
  ...
  try {
    ...
    const parsed = JSON.parse(stdout);
    const items: MarketplaceSkill[] = parsed.items || [];
    cache.marketplace = { data: items, ts: Date.now() };
    return items;
  } catch (err) {
    if (cache.marketplace) return cache.marketplace.data;
...
function mergeSkills(runtime: RuntimeSkill[], marketplace: MarketplaceSkill[]): MergedSkill[] {
...
skillsRouter.get('/search', async (req, res) => {
  ...
  try {
    ...
    const data = results.map(m => ({
      ...
      name: m.displayName,
      description: m.summary,
      source: runtimeSlugs.has(m.slug) ? 'both' as const : 'marketplace' as const,
      status: runtimeSlugs.has(m.slug) ? 'ready' as const : 'not-installed' as const,
