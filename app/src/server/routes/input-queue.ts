...
async function taskBelongsToUser(userId: string, taskId: string): Promise<boolean> {
  const [row] = await db
    .select({ id: tasks.id })
    .from(tasks)
...
async function agentBelongsToUser(userId: string, agentId: string): Promise<boolean> {
  const [row] = await db
    .select({ id: agents.id })
    .from(agents)
...
async function emitPendingCountChanged(userId: string): Promise<void> {
  const pendingCount = await getPendingCount(userId);
  emitEvent(userId, 'input_queue', 'input_queue.count_changed', { pendingCount });
}

inputQueueRouter.get(
  '/',
  asyncHandler(async (req, res) => {
    const conditions: SQL<unknown>[] = [eq(inputQueue.userId, req.userId)];
    ...
    if (status) {
      ...
      if (statuses.length === 1) {
        conditions.push(eq(inputQueue.status, statuses[0] as string));
...
inputQueueRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [row] = await db
      .select()
      .from(inputQueue)
      .where(and(eq(inputQueue.id, req.params.id as string), eq(inputQueue.userId, req.userId)))
      .limit(1);
...
inputQueueRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(inputQueue).values(insertValues as any).returning();
