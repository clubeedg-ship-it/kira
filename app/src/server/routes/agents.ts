...
function respondOrchestratorError(
  error: unknown,
  handlers: {
    validation: (message: string) => void;
    notFound: (message: string) => void;
    capacity: (message: string) => void;
  },
): boolean {
...
agentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [agent] = await db
      .select()
      .from(agents)
      .where(and(eq(agents.id, req.params.id as string), eq(agents.userId, req.userId)))
      .limit(1);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (requestedTaskId) {
      try {
        ...
        if (
          respondOrchestratorError(error, {
    ...
    const candidateTasks = await db
      ...
      .where(
        and(
          ...
          sql`${tasks.executorId} IS NULL OR ${tasks.executorId} = ${agentId}`,
        ),
      )
...
agentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [completedTodayRow, costTodayRow, approvalRow] = await Promise.all([
      db
        .select({
          count: sql<number>`count(distinct ${agentWorkLog.taskId})::int`,
        })
        .from(agentWorkLog)
        .where(
          and(
            eq(agentWorkLog.userId, req.userId),
            eq(agentWorkLog.agentId, agentId),
            eq(agentWorkLog.action, 'task_completed'),
            gte(agentWorkLog.createdAt, startOfTodayIso),
          ),
        )
        .then((rows) => rows[0]),
      db
        .select({
          total: sql<number>`coalesce(sum(${agentWorkLog.costUsd}), 0)::float`,
        })
        .from(agentWorkLog)
        .where(
          and(
      ...
      db
        .select({
          ...
          approvedCount: sql<number>`sum(case when ${inputQueue.status} = 'resolved' then 1 else 0 end)::int`,
        })
        .from(inputQueue)
        .where(
          and(
            eq(inputQueue.userId, req.userId),
            eq(inputQueue.agentId, agentId),
            inArray(inputQueue.status, ['resolved', 'dismissed']),
          ),
        )
        .then((rows) => rows[0]),
    ]);
...
agentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(agents).values(insertValues as any).returning();
...
agentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = await db
      .select()
      .from(agentWorkLog)
      .where(and(eq(agentWorkLog.agentId, req.params.id as string), eq(agentWorkLog.userId, req.userId)))
  ...
  asyncHandler(async (_req, res) => {
    try {
      ...
      const subagents = (sessions.sessions || sessions || [])
        .filter((s: any) => s.kind === 'sub-agent' || s.key?.includes('subagent'))
        .map((s: any) => ({
