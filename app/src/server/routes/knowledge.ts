...
knowledgeRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!type) return validationError(res, 'type is required');
...
knowledgeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!isUuid(id)) return validationError(res, 'Invalid id');
    ...
    const rels = await db
      ...
      .where(
        and(
          eq(relationships.userId, req.userId),
          or(eq(relationships.sourceEntityId, id), eq(relationships.targetEntityId, id)),
        ),
      );
...
knowledgeRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (hasOwn(body, 'name')) {
      const name = getTrimmedString(body.name);
      if (!name) return validationError(res, 'name cannot be empty');
      updates.name = name;
    }
    if (hasOwn(body, 'type')) updates.type = body.type;
    if (hasOwn(body, 'aliases')) updates.aliases = body.aliases;
    if (hasOwn(body, 'properties')) updates.properties = body.properties;
    ...
    if (!row) return notFound(res, 'Entity not found');
    success(res, row);
  }),
);
...
knowledgeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (type && type !== 'all') whereParts.push(sql`e.type = ${type}`);
    if (search) whereParts.push(sql`e.name ILIKE ${`%${search}%`}`);
...
knowledgeRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!entityId || !isUuid(entityId)) return validationError(res, 'Valid entityId is required');
    if (!key) return validationError(res, 'key is required');
    if (!value) return validationError(res, 'value is required');
