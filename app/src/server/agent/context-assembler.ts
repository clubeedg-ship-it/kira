...
function classifyQuery(message: string): Classification {
  const lower = message.toLowerCase();
  const scores: Record<Classification, number> = { technical: 0, personal: 0, task: 0, creative: 0, question: 0, general: 0 };

  for (const [cat, keywords] of Object.entries(CLASSIFICATION_KEYWORDS) as [Classification, string[]][]) {
    for (const kw of keywords) {
      if (lower.includes(kw)) scores[cat]++;
    }
  }

  let best: Classification = 'general';
  let bestScore = 0;
  for (const [cat, score] of Object.entries(scores) as [Classification, number][]) {
...
function selectIdentityKeys(classification: Classification): string[] {
...
async function queryRelevantMemories(userId: string, keywords: string[]): Promise<{ content: string; importance: number }[]> {
  if (keywords.length === 0) return [];

  const now = new Date().toISOString();
  const results: { content: string; importance: number }[] = [];
  ...
  for (const kw of keywords.slice(0, 5)) {
    try {
      const rows = await db
        ...
        .where(
          and(
            eq(memoryShortTerm.userId, userId),
            sql`(${memoryShortTerm.expiresAt} IS NULL OR ${memoryShortTerm.expiresAt} > ${now})`,
            like(memoryShortTerm.content, `%${kw}%`),
          ),
        )
...
function trimTobudget(parts: { label: string; text: string; priority: number }[]): { included: string[]; text: string } {
...
export async function assembleContext(userId: string, userMessage?: string): Promise<string>;
export async function assembleContext(userId: string, userMessage: string, withMetadata: true): Promise<AssembledContext>;
export async function assembleContext(userId: string, userMessage?: string, withMetadata?: boolean): Promise<string | AssembledContext> {
  ...
  let graphEntities: { entities: any[]; facts: any[] } = { entities: [], facts: [] };
  ...
  const settingsJson = (settings.settings || {}) as Record<string, string>;

  // 6. Build parts with priorities (lower = higher priority)
  const parts: { label: string; text: string; priority: number }[] = [];
