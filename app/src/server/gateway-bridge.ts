...
let currentActivity: AgentActivity = { state: 'idle', updatedAt: Date.now() };
let rpcIdCounter = 0;
...
function connect() {
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (msg.type === 'res') {
        if (msg.ok) console.log('[gateway-bridge] Authenticated successfully');
        else console.log('[gateway-bridge] Connect rejected:', JSON.stringify(msg.error || msg).slice(0, 200));
        return;
      }
  ...
  ws.on('error', (err) => {
    console.log('[gateway-bridge] WS error:', (err as Error).message);
  });
}
...
export function notifyStreamStart(runId: string) {
  setActivity({ state: 'thinking', runId, startedAt: Date.now(), text: undefined, toolName: undefined });
}
export function notifyStreamDelta(runId: string, accumulatedText: string) {
...
export function getActivity(): AgentActivity { return { ...currentActivity }; }

export function addSSEClient(res: import('express').Response): string {
