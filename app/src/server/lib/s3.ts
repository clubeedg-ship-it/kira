...
function getStatusCode(error: unknown): number | undefined {
  ...
  return Number((error as { $metadata?: { httpStatusCode?: number } }).$metadata?.httpStatusCode);
}

function shouldFallback(statusCode: number | undefined): boolean {
  return (
    statusCode === 301 ||
    statusCode === 302 ||
    statusCode === 307 ||
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          console.warn(
            `S3 unavailable in development mode; falling back to local storage at ${LOCAL_FALLBACK_DIR}.`,
          );
          return;
        }
