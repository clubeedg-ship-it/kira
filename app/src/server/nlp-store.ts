...
export async function storeExtractions(
  ...
  const resolveEntityId = async (name: string): Promise<string | null> => {
    ...
    if (entityIdMap.has(key)) return entityIdMap.get(key)!;
      ...
      const [existing] = await db
        ...
        .where(
          and(
            ...
            eq(relationships.sourceEntityId, sourceId),
            eq(relationships.targetEntityId, targetId),
