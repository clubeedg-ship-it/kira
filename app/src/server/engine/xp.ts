...
export function calculateTaskXP(task: { durationEst?: number | null }): number {
  ...
  let complexity = 0;
  if (dur > 180) complexity = 4;
  else if (dur > 60) complexity = 3;
  else if (dur > 30) complexity = 2;
  else if (dur > 15) complexity = 1;
...
export async function checkStreak(userId: string): Promise<void> {
  ...
  if (record.lastActiveDate === today) return; // Already checked in today
  ...
  if (record.lastActiveDate === yesterdayKey || record.lastActiveDate === null) {
    ...
    const daysSinceActive = Math.floor(
      (new Date(today + 'T00:00:00').getTime() - new Date(record.lastActiveDate + 'T00:00:00').getTime()) / (1000 * 60 * 60 * 24)
    );
