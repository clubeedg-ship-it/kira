...
async function recalculateObjectiveProgress(userId: string, objectiveId: string): Promise<void> {
  const rows = await db
    .select({
...
async function cancelTasksForProjects(userId: string, projectIds: string[]): Promise<void> {
...
export async function onKeyResultUpdate(userId: string, keyResultId: string): Promise<void> {
...
export async function onParentArchived(
  userId: string,
  entityType: 'area' | 'objective' | 'project',
  entityId: string,
): Promise<void> {
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(projects.objectiveId, entityId),
  ...
  await db
    ...
    .where(
      and(
        ...
        eq(objectives.areaId, entityId),
        ne(objectives.status, 'archived'),
      ),
    );
