...
import { AGENT_WORK_QUEUE_NAME, HEARTBEAT_QUEUE_NAME, createRedisConnection, } from './jobs/queue';
...
function registerWorkerEvents(worker, queueName) {
    worker.on('ready', () => {
    ...
    worker.on('failed', (job, error) => {
        console.error(`[worker] failed job ${job?.id ?? 'unknown'} (${job?.name ?? queueName}):`, error.message);
    });
    worker.on('error', (err) => {
