 ...
 */
export async function findMatchingPattern(userId, input) {
    try {
        ...
        for (const pattern of patterns) {
            ...
            if (pattern.intentType) {
                const patternWords = new Set((pattern.inputPattern || pattern.intentType).toLowerCase().split(/\s+/).filter((w) => w.length > 2));
 ...
 */
export async function updatePatternScore(patternId, success) {
    try {
        if (success) {
            await db
 ...
 */
export async function createPattern(userId, intentType, input, template) {
    try {
...
//# sourceMappingURL=prompt-patterns.js.map