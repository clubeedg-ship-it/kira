...
function extractEntities(text) {
    ...
    const add = (name, type, confidence) => {
    ...
    while ((m = capitalizedPattern.exec(text)) !== null) {
        ...
        if (FALSE_POSITIVE_PHRASES.has(lower))
            continue;
    ...
    while ((m = singleCapPattern.exec(text)) !== null) {
        const word = m[1];
        if (COMMON_SENTENCE_STARTERS.has(word.toLowerCase()))
            continue;
...
function extractFacts(text) {
    const facts = [];
    const seen = new Set();
    const addFact = (subject, key, value, confidence) => {
        subject = subject.trim();
        value = value.trim().replace(/[.!?,;:]+$/, '').trim();
        if (!subject || !value || value.length < 2 || value.length > 200)
            return;
    ...
    while ((m = isPattern.exec(text)) !== null) {
        const subj = m[1].trim();
        if (FALSE_POSITIVE_PHRASES.has(subj.toLowerCase()))
            continue;
        if (COMMON_SENTENCE_STARTERS.has(subj.toLowerCase()))
            continue;
...
function extractRelations(text) {
    const relations = [];
    const seen = new Set();
    const addRel = (source, target, type, confidence) => {
        source = source.trim();
        target = target.trim().replace(/[.!?,;:]+$/, '').trim();
        if (!source || !target || target.length < 2)
            return;
...
export async function extractWithLLM(text, apiKey, model, baseUrl = 'https://api.openai.com/v1') {
    if (!text || !apiKey) {
...
//# sourceMappingURL=nlp-extract.js.map