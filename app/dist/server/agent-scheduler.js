...
export async function scheduleAgent(agent) {
    ...
    await queue.add('run-agent', { agentId: agent.id, userId: agent.userId }, {
...
export async function unscheduleAgent(agentId) {
    const queue = getQueue();
    try {
...
export async function runAgentNow(agentId, userId) {
    const queue = getQueue();
...
export async function initializeAgentSchedules() {
