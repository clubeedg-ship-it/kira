...
export async function searchEntities(userId, query, limit) {
    try {
        return await db
            .select()
            .from(entities)
...
export async function getEntityFacts(userId, entityName) {
    try {
...
export async function getRelatedEntities(userId, entityName) {
    try {
        const matched = await db
            .select()
            .from(entities)
        ...
        return await db
            .select()
            .from(relationships)
            .where(and(eq(relationships.userId, userId), sql `(${relationships.sourceEntityId} = ${matched[0].id} OR ${relationships.targetEntityId} = ${matched[0].id})`));
    }
    catch {
        return [];
    }
}
export async function queryContextForText(userId, text) {
    const result = {
        entities: [],
        facts: [],
        relations: [],
            ...
            for (const ent of matched) {
                if (!seenEntityIds.has(ent.id)) {
                    ...
                    const rels = await db
                        .select()
                        .from(relationships)
                        .where(and(eq(relationships.userId, userId), sql `(${relationships.sourceEntityId} = ${ent.id} OR ${relationships.targetEntityId} = ${ent.id})`));
