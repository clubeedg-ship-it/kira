...
export async function runMaintenance(userId) {
    ...
    try {
        ...
        const result = await db
            ...
            .where(and(eq(facts.userId, userId), lt(facts.updatedAt, thirtyDaysAgo)))
    ...
    try {
        ...
        const result = await db
            ...
            .where(and(eq(memoryStaging.userId, userId), eq(memoryStaging.status, 'pending'), sql `(${memoryStaging.data}->>'confidence')::float > 0.8`))
