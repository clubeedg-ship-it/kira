...
projectsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(projects).values(insertValues).returning();
...
projectsRouter.post('/:id/milestones', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
    if (!(await projectBelongsToUser(req.userId, req.params.id))) {
        notFound(res, 'Project not found');
        return;
    }
    const body = asRecord(req.body);
    ...
    const [created] = await db.insert(milestones).values(insertValues).returning();
    success(res, created, 201);
}));
milestonesRouter.put('/:id', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
    ...
    const [updated] = await db
        .update(milestones)
        .set(updateValues)
        .where(and(eq(milestones.id, req.params.id), eq(milestones.userId, req.userId)))
        .returning();
    if (!updated) {
