...
import { asyncHandler, getQueryString, getTrimmedString, isUuid, notFound, success, validationError, } from './utils';
...
documentsRouter.get('/', asyncHandler(async (req, res) => {
    ...
    if (folder)
        conditions.push(eq(documents.folder, folder));
    if (search) {
        conditions.push(or(ilike(documents.title, `%${search}%`), ilike(documents.content, `%${search}%`)));
    }
    const rows = await db
        .select()
        .from(documents)
...
documentsRouter.get('/:id', asyncHandler(async (req, res) => {
    ...
    if (!isUuid(id))
        return validationError(res, 'Invalid document id');
    const [doc] = await db
        .select()
        .from(documents)
...
documentsRouter.put('/:id', asyncHandler(async (req, res) => {
    const id = req.params.id;
    if (!isUuid(id))
        return validationError(res, 'Invalid document id');
    const updates = {
    ...
    if (req.body.title !== undefined)
        updates.title = getTrimmedString(req.body.title);
    if (req.body.content !== undefined)
        updates.content = req.body.content;
    if (req.body.folder !== undefined)
        updates.folder = getTrimmedString(req.body.folder);
    if (req.body.tags !== undefined)
        updates.tags = Array.isArray(req.body.tags) ? req.body.tags : null;
    if (req.body.mimeType !== undefined)
        updates.mimeType = getTrimmedString(req.body.mimeType);
    if (req.body.summary !== undefined)
        updates.summary = getTrimmedString(req.body.summary);
    const [doc] = await db
        .update(documents)
        .set(updates)
        .where(and(eq(documents.id, id), eq(documents.userId, req.userId)))
