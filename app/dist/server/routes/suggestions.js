...
suggestionsRouter.get('/', async (req, res) => {
    try {
        const rows = await db
            ...
            .where(and(eq(extractedSuggestions.userId, req.userId), eq(extractedSuggestions.status, 'pending')))
...
suggestionsRouter.post('/:id/accept', async (req, res) => {
    try {
        const [updated] = await db
            ...
            .set({ status: 'accepted' })
            .where(and(eq(extractedSuggestions.id, req.params.id), eq(extractedSuggestions.userId, req.userId)))
            .returning();
        if (!updated) {
