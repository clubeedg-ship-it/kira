...
import { asRecord, asyncHandler, getInteger, getNumber, getQueryString, getTrimmedString, hasOwn, isDateOnly, isUuid, notFound, success, validationError, } from './utils';
...
async function objectiveBelongsToUser(userId, objectiveId) {
    const [row] = await db
...
objectivesRouter.get('/', asyncHandler(async (req, res) => {
    const conditions = [eq(objectives.userId, req.userId)];
...
objectivesRouter.get('/:id', asyncHandler(async (req, res) => {
    ...
    const [objective] = await db
        .select()
        .from(objectives)
        .where(and(eq(objectives.id, req.params.id), eq(objectives.userId, req.userId)))
        .limit(1);
    if (!objective) {
...
objectivesRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(objectives).values(insertValues).returning();
    success(res, created, 201);
}));
objectivesRouter.put('/:id', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
...
objectivesRouter.post('/:id/key-results', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
    if (!(await objectiveBelongsToUser(req.userId, req.params.id))) {
        notFound(res, 'Objective not found');
        return;
    }
    const body = asRecord(req.body);
    ...
    const [created] = await db.insert(keyResults).values(insertValues).returning();
    success(res, created, 201);
}));
keyResultsRouter.put('/:id', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
    ...
    const [updated] = await db
        .update(keyResults)
        .set(updateValues)
        .where(and(eq(keyResults.id, req.params.id), eq(keyResults.userId, req.userId)))
        .returning();
    if (!updated) {
...
keyResultsRouter.delete('/:id', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
