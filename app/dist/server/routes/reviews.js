...
import { asRecord, asyncHandler, getQueryString, getTrimmedString, hasOwn, isIsoDateTime, isUuid, notFound, success, validationError, } from './utils';
const reviewsRouter = Router();
reviewsRouter.get('/', asyncHandler(async (req, res) => {
    const conditions = [eq(reviews.userId, req.userId)];
...
reviewsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(reviews).values(insertValues).returning();
    success(res, created, 201);
}));
reviewsRouter.put('/:id', asyncHandler(async (req, res) => {
    ...
    const [updated] = await db
        .update(reviews)
        .set(updateValues)
        .where(and(eq(reviews.id, req.params.id), eq(reviews.userId, req.userId)))
        .returning();
    if (!updated) {
...
reviewsRouter.post('/generate', asyncHandler(async (req, res) => {
    const body = asRecord(req.body);
    const reviewType = getTrimmedString(body.reviewType ?? body.review_type) ||
        getQueryString(req.query.review_type) ||
        getQueryString(req.query.type) ||
        'daily';
    const targetDate = getTrimmedString(body.date) ||
