...
import { agents, areas, inputQueue, keyResults, milestones, objectives, projects, tasks, users, } from '../../db/schema';
import { isTaskStatus } from '../engine/state-machine';
...
const boardStatusSet = new Set(boardStatuses);
...
const commandCenterAgentStatusOrder = {
    working: 0,
    idle: 1,
};
...
function extractDaysBetween(startDate, endDate) {
...
function getCurrentQuarterLabel(dateValue) {
...
viewsRouter.get('/board', asyncHandler(async (req, res) => {
    ...
    const parsedPriority = priorityFilterRaw && Number.isInteger(Number(priorityFilterRaw))
        ? Number(priorityFilterRaw)
        : null;
    const priorityFilter = parsedPriority !== null && validPriorities.has(parsedPriority)
        ? parsedPriority
        : null;
    ...
    const boardRows = await db
        ...
        .from(tasks)
        .leftJoin(projects, and(eq(projects.id, tasks.projectId), eq(projects.userId, req.userId)))
        .leftJoin(milestones, and(eq(milestones.id, tasks.milestoneId), eq(milestones.userId, req.userId)))
    ...
    const projectConditions = [eq(projects.userId, req.userId)];
    if (areaFilter) {
...
viewsRouter.get('/command-center', asyncHandler(async (req, res) => {
    const now = new Date();
    const todayKey = toDateKey(now);
    const dueSoonKey = toDateKey(addDays(now, 2));
    const staleInputCutoffIso = addDays(now, -5).toISOString();
    const [userRows, taskRows, queueRows, agentRows, allActiveObjectives, completionRows,] = await Promise.all([
        db
            .select({
            name: users.name,
        })
    ...
    const typedTaskRows = taskRows;
    const typedQueueRows = queueRows;
    const typedAgentRows = agentRows;
    const typedObjectives = allActiveObjectives;
    ...
    const inboxBreakdownMap = new Map();
    ...
    const inProgressTaskByAgentId = new Map();
    for (const task of activeTasks) {
    ...
    const queueTaskByAgentId = new Map();
    for (const item of typedQueueRows) {
    ...
    if (activeAgents.length === 0 && typedQueueRows.length > 0) {
        ...
        activeAgents.push(...[...fallbackAgents.values()].slice(0, 5).map((agent) => ({
            ...agent,
            status: 'waiting',
        })));
    }
    const currentQuarter = getCurrentQuarterLabel(now);
    const currentQuarterObjectives = typedObjectives.filter((objective) => objective.quarter === currentQuarter);
    const selectedObjectives = currentQuarterObjectives.length > 0
    ...
    if (selectedObjectiveIds.length > 0) {
        ...
        selectedKeyResults = keyResultRows;
    }
    ...
    const keyResultProgress = selectedObjectives.map((objective) => {
        ...
        const derivedProgress = objectiveKeyResults.length > 0
            ? clampPercent(objectiveKeyResults.reduce((total, item) => total + item.progress, 0) /
    ...
    const hasData = typedTaskRows.length > 0 ||
