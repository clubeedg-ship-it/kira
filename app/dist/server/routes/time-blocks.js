...
import { asRecord, asyncHandler, getBoolean, getInteger, getQueryString, getTrimmedString, hasOwn, isDateOnly, isUuid, notFound, success, validationError, } from './utils';
...
timeBlocksRouter.get('/', asyncHandler(async (req, res) => {
    const conditions = [eq(timeBlocks.userId, req.userId)];
...
timeBlocksRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(timeBlocks).values(insertValues).returning();
    success(res, created, 201);
}));
timeBlocksRouter.put('/:id', asyncHandler(async (req, res) => {
    ...
    const [updated] = await db
        .update(timeBlocks)
        .set(updateValues)
        .where(and(eq(timeBlocks.id, req.params.id), eq(timeBlocks.userId, req.userId)))
        .returning();
    if (!updated) {
...
timeBlocksRouter.delete('/:id', asyncHandler(async (req, res) => {
    if (!isUuid(req.params.id)) {
        validationError(res, 'id must be a valid UUID');
        return;
    }
