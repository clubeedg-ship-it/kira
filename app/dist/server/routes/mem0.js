...
import { searchMemory, getAllMemories, getMemoryById, deleteMemory, getMemoryHistory, addToMemory, } from '../memory/mem0-service';
...
mem0Router.get('/memories', async (req, res) => {
    try {
        ...
        const agentId = req.query.agentId || 'kira';
...
mem0Router.post('/add', async (req, res) => {
    try {
        ...
        const result = await addToMemory([{ role: 'user', content }], { userId, agentId: agentId || 'kira' });
