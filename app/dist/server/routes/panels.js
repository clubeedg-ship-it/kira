...
panelsRouter.get('/', asyncHandler(async (req, res) => {
    const rows = await db
        .select()
        .from(chatPanels)
...
panelsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const maxPos = await db
        .select({ max: sql `coalesce(max(${chatPanels.position}), -1)` })
        .from(chatPanels)
        .where(and(eq(chatPanels.userId, req.userId), eq(chatPanels.isActive, true)));
...
panelsRouter.delete('/:id', asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!isUuid(id)) {
        validationError(res, 'invalid panel id');
        return;
    }
    const [updated] = await db
        ...
        .returning();
    if (!updated) {
        notFound(res, 'panel not found');
        return;
    }
    success(res, updated);
}));
/* POST /api/v1/panels/:id/branch — memory branch point (placeholder) */
panelsRouter.post('/:id/branch', asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!isUuid(id)) {
        validationError(res, 'invalid panel id');
        return;
