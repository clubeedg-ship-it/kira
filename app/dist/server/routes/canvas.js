...
canvasRouter.get('/', asyncHandler(async (req, res) => {
    const rows = await db
        .select()
        .from(canvasStates)
...
// Get canvas by ID
canvasRouter.get('/:id', asyncHandler(async (req, res) => {
...
// Create canvas
canvasRouter.post('/', asyncHandler(async (req, res) => {
    const { type, title, content, conversationId } = req.body;
    if (!type || !content)
        return validationError(res, 'type and content required');
    const [row] = await db
        .insert(canvasStates)
        .values({
        userId: req.userId,
...
// Update canvas
canvasRouter.put('/:id', asyncHandler(async (req, res) => {
    const id = req.params.id;
...
canvasRouter.post('/:id/respond', asyncHandler(async (req, res) => {
    const id = req.params.id;
    const { response } = req.body;
    if (!response)
        return validationError(res, 'response required');
    const [row] = await db
        .update(canvasStates)
        .set({
        response,
...
// Dismiss canvas
canvasRouter.delete('/:id', asyncHandler(async (req, res) => {
    const id = req.params.id;
    const [row] = await db
        .update(canvasStates)
