...
import { asRecord, asyncHandler, getNumber, getQueryString, getTrimmedString, hasOwn, isDateOnly, isUuid, notFound, success, validationError, } from './utils';
...
async function principleBelongsToUser(userId, principleId) {
    const [row] = await db
...
principlesRouter.get('/', asyncHandler(async (req, res) => {
    const conditions = [eq(principles.userId, req.userId)];
...
principlesRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(principles).values(insertValues).returning();
    success(res, created, 201);
}));
principlesRouter.put('/:id', asyncHandler(async (req, res) => {
    ...
    const [updated] = await db
        .update(principles)
        .set(updateValues)
        .where(and(eq(principles.id, req.params.id), eq(principles.userId, req.userId)))
        .returning();
    if (!updated) {
...
decisionsRouter.get('/', asyncHandler(async (req, res) => {
    const conditions = [eq(decisions.userId, req.userId)];
...
decisionsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const [created] = await db.insert(decisions).values(insertValues).returning();
    success(res, created, 201);
}));
