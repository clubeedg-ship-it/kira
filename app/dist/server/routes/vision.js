...
import { asRecord, asyncHandler, getTrimmedString, hasOwn, success, validationError, } from './utils';
const visionRouter = Router();
visionRouter.get('/', asyncHandler(async (req, res) => {
    const [row] = await db
        .select()
        .from(vision)
...
visionRouter.put('/', asyncHandler(async (req, res) => {
    ...
    if (!existing) {
        const [created] = await db
            .insert(vision)
            .values({ userId: req.userId, ...upsertValues })
            .returning();
