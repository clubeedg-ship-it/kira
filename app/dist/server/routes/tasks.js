...
import { asRecord, asyncHandler, getInteger, getNumber, getQueryString, getTrimmedString, hasOwn, isDateOnly, isIsoDateTime, isUuid, notFound, success, validationError, } from './utils';
const tasksRouter = Router();
async function projectBelongsToUser(userId, projectId) {
    const [row] = await db
...
async function milestoneBelongsToUser(userId, milestoneId) {
    const [row] = await db
...
async function timeBlockBelongsToUser(userId, timeBlockId) {
    const [row] = await db
...
async function getTaskBlockingCount(userId, taskId) {
    const [row] = await db
...
tasksRouter.get('/today', asyncHandler(async (req, res) => {
    const today = new Date().toISOString().slice(0, 10);
    const rows = await db
        .select()
        .from(tasks)
        .where(and(eq(tasks.userId, req.userId), or(eq(tasks.scheduledDate, today), eq(tasks.dueDate, today))))
        .orderBy(desc(tasks.priorityScore), asc(tasks.priority), asc(tasks.createdAt));
    success(res, rows);
}));
tasksRouter.get('/inbox', asyncHandler(async (req, res) => {
    const rows = await db
        .select()
        .from(tasks)
...
tasksRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const priorityForScore = typeof insertValues.priority === 'number' ? insertValues.priority : 2;
    const dueDateForScore = typeof insertValues.dueDate === 'string' ? insertValues.dueDate : null;
    insertValues.priorityScore = calculatePriority({
        priority: priorityForScore,
        dueDate: dueDateForScore,
    }, 0);
    const [created] = await db.insert(tasks).values(insertValues).returning();
...
tasksRouter.put('/:id', asyncHandler(async (req, res) => {
    ...
    const nextPriority = hasOwn(updateValues, 'priority') && typeof updateValues.priority === 'number'
        ? updateValues.priority
        : existingTask.priority;
    const nextDueDate = hasOwn(updateValues, 'dueDate') &&
