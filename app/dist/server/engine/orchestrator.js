...
function prepareAgents(rows, activeCountByAgentId) {
    return rows
...
function scoreAgentForTask(task, agent) {
    ...
    const overlapCount = requiredCapabilities.length === 0
        ? 0
...
function getBestAgentForTask(task, preparedAgents) {
    let bestMatch = null;
...
async function getActiveTaskCountByAgent(userId) {
    const rows = await db
        .select({
        ...
        .from(tasks)
        .where(and(eq(tasks.userId, userId), eq(tasks.executorType, 'agent'), eq(tasks.status, 'in_progress'), isNotNull(tasks.executorId)))
...
async function listAgents(userId, onlyIdle = false) {
...
async function listAssignableTasks(userId) {
    return db
        .select({
        id: tasks.id,
        title: tasks.title,
        ...
        .where(and(eq(tasks.userId, userId), eq(tasks.executorType, 'agent'), eq(tasks.status, 'todo')))
        .orderBy(desc(tasks.priorityScore), asc(tasks.priority), asc(tasks.createdAt));
}
async function getTaskCandidateById(userId, taskId) {
    const [row] = await db
        .select({
        id: tasks.id,
        title: tasks.title,
...
async function filterUnblockedTasks(userId, candidateTasks) {
    ...
    const dependencyRows = await db
        ...
        .from(dependencies)
        .where(and(eq(dependencies.userId, userId), eq(dependencies.blockerType, 'task'), eq(dependencies.blockedType, 'task'), inArray(dependencies.blockedId, candidateTaskIds)));
...
export async function getAgentForTask(userId, taskId) {
...
export async function assignAvailableTasks(userId) {
...
export async function enqueueAgentTask(userId, agentId, taskId) {
    ...
    if (!unblockedTask) {
        throw new OrchestratorError('VALIDATION_ERROR', 'Task is blocked by unresolved dependencies');
    }
    ...
    if (preparedAgent.availableSlots <= 0) {
        throw new OrchestratorError('CAPACITY_REACHED', 'Agent has reached max_concurrent capacity');
    }
    const score = scoreAgentForTask(task, preparedAgent);
    if (score === null) {
        throw new OrchestratorError('VALIDATION_ERROR', 'Task does not match the selected agent capabilities or area assignments');
    }
