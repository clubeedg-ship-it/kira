...
import { entities, facts, userPreferences, promptLog, } from '../db/schema';
...
async function resolveReferences(userId, input, history) {
    try {
        ...
        const parts = [];
        if (matched.length > 0)
            parts.push(`Entities: ${matched.join(', ')}`);
        if (resolvedEntities.length > 0)
            parts.push(`Facts: ${resolvedEntities.join('; ')}`);
        if (contextFromHistory)
            parts.push(`Recent context available`);
...
function getChainOfThought(intent) {
...
function buildStructuredPrompt(input, intent, resolvedContext, prefs, cot, history) {
    const sections = [];
...
async function llmResolve(input, history, apiKey, model) {
        ...
        const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Given user input and conversation context, resolve references and classify intent. Return JSON: {"resolvedInput": "...", "intent": "fix|create|analyze|explain|search|refactor|test|summarize|general"}',
                    },
                    {
                        role: 'user',
                        content: `Input: "${input}"\n\nRecent conversation:\n${recentHistory || 'None'}`,
...
export async function enhancePrompt(userId, rawInput, conversationHistory, apiKey, model) {
    ...
    try {
        ...
        // Stage 1: Reference Resolution
        const { resolved, description: refDesc } = await resolveReferences(userId, rawInput, conversationHistory);
        ...
        // Stage 5: Structured Output
        const enhancedPrompt = buildStructuredPrompt(resolved, intent, refDesc, prefs, cot, conversationHistory);
...
// ── Stage 6: Scoring ─────────────────────────────────────────────────────────
export async function scorePrompt(logId, outcome, feedback) {
    try {
        await db
            .update(promptLog)
            .set({
...
// ── Helpers ──────────────────────────────────────────────────────────────────
async function logEnhancement(userId, rawInput, enhancedPrompt, latencyMs) {
    const [row] = await db
        .insert(promptLog)
        .values({
        userId,
