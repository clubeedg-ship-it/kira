...
import { boolean, date, index, integer, jsonb, pgTable, primaryKey, real, text, time, timestamp, unique, uniqueIndex, uuid, varchar, } from 'drizzle-orm/pg-core';
...
export const dependencies = pgTable('dependencies', {
    ...
    blockerBlockedUnique: uniqueIndex('dependencies_blocker_blocked_unique').on(table.blockerId, table.blockedId),
}));
...
export const inputQueue = pgTable('input_queue', {
    id: uuid('id').primaryKey().default(sql `gen_random_uuid()`),
    userId: uuid('user_id')
        .notNull()
        .references(() => users.id),
...
export const extractedSuggestions = pgTable('extracted_suggestions', {
    id: uuid('id').primaryKey().default(sql `gen_random_uuid()`),
    userId: uuid('user_id')
        .notNull()
        .references(() => users.id),
